...
const handleCrearImgComplete = (p_src, p_alt, p_title, p_id, p_data_target, p_user, p_offset,p_typePage) => { 
    ...
    divGrid.addEventListener('mouseenter', (e) => {
        handleCreateButtonFDO(divGrid, p_data_target, p_id,p_user,p_title);
...
const handleFavorite = (e) => {
    ...
    const FAVORITO = 'FAV:';
    const FAVOPEN='FAVOPEN:';
    e.classList.toggle("btnactive");
    let id = e.id;
    let idEnd = id.substring(FAVORITO.length, id.length);
    const idOpen=id.substring(FAVOPEN.length, id.length);
    
    let btnSTGActive = document.getElementById(id);
    
    if (btnSTGActive.classList[3] === "btnactive") {
        let imgData = document.getElementById(idEnd)||document.getElementById(idOpen);    
        let classDataImage = new DataImage(idEnd, imgData.src, imgData.alt, imgData.title, imgData.dataset.targetUsername,imgData.dataset.targetOffset,'mainFavorites'/* imgData.dataset.typePage */) ;
        saveLocalStorage(classDataImage);
    } else {
        removeLocalStorage(idOpen);
...
const handleOpen = (e) => {
    ...
    let v_key = e.target.id.substring(OPEN.length, e.target.id.length);
